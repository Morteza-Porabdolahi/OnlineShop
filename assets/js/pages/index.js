...
import { addUserWish, isProductInWishlist, deleteUserWish } from "../api/wishlist";
import { handleNavbarNumbers } from "./general";
...
function handleHeaderSearchInput(e) {
    const newHref = `/pages/shopPage.html?q=${headerSearchInput.value}?category=${categoryTitle.dataset.category || ""}`;
...
async function getAllProducts() {
    try {
        // I'm going to implement some endpoints for api like /products/bestselling or /products/new and that's because i use the Promise.allSettled
...
function createNewAndEssentialElems(products = [], containerClass = "") {
    ...
    products.forEach(({ discount, price, imageUrl, description, title, _id }) => {
        ...
        productTemplate.querySelector('.like-btn').addEventListener('click', (e) => handleUserWish(e, _id, containerClass));
...
async function handleUserWish(event, productId = "", containerClass = "") {
    ...
    let likeButton = $$.querySelector(`${containerClass} .like-btn`);
