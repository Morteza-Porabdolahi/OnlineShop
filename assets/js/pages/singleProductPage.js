...
(function () {
	...
	tabButtons.forEach(tabButton => tabButton.addEventListener('click', handleSwitchTab));
	starElems.forEach(starElem => starElem.addEventListener('click', handleScoreStars));
	...
	async function getSingleProduct() {
		...
		try {
			...
			insertProductDatasIntoPage(product);
		} catch (e) {
			console.log(e);
		}
	}
	...
	function insertProductDatasIntoPage(product = {}) {
		$$.querySelector('.intro__breadcrumb > .item--active').textContent = product.title;
		$$.querySelector('.intro__title').textContent = product.title;

		$$.querySelectorAll('.product-additional-desc__price').forEach(priceElem => {
		...
		$$.querySelectorAll('.product-additional-desc__add-to-basket-btn').forEach(addToBasket => {
			addToBasket.addEventListener('click', () => addItemInUserCart(product._id));
		})
	}
})()